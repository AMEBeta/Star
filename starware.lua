...
local UILibrary = loadstring(game:HttpGet("https://raw.githubusercontent.com/AMEBeta/Star/refs/heads/main/starwareui.lua",true))();
 ...
 local GetConfig = function()
    local read, data = pcall(readfile, "arpstarware.json");
     ...
     if (read and canDecode) then
         ...
         if (Decoded.Version ~= DefaultSettings.Version) then
             ...
             writefile("arpstarware.json", Encoded);
 ...
 local MainUI = UILibrary.new(Color3.fromRGB(204, 204, 0));
 local Window = MainUI:LoadWindow('<font color="#EFBF04">arpstarware</font> Lua', UDim2.fromOffset(400, 279));
 
 -- Initialize the UI with the purple theme
 local MainUI = UILibrary.new(Color3.fromRGB(204, 204, 0)); -- Hex #4307f1
 local Window = MainUI:LoadWindow('<font color="#EFBF04">arpstarware</font> Lua', UDim2.fromOffset(400, 279));
 ...
 while wait(5) do
     ...
     writefile("AMEBeta.json", Encoded);
 end
